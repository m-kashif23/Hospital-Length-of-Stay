...
The trained model `XGboost.pkl` is downloaded from the Hugging Face Hub at
runtime (it is too big for GitHub). Only app.py + requirements.txt need to live
in your GitHub repo. Set HF_REPO_ID below to match your own HF model repo.
"""
...
# >>> EDIT THIS to match the Hugging Face repo where you uploaded XGboost.pkl <<<
HF_REPO_ID  = "Mkashif23/los-xgboost"
HF_FILENAME = "XGboost.pkl"
...
# --------------------------------------------------------------------------- #
# Load model (from Hugging Face) + decode the categorical encoder it carries
# --------------------------------------------------------------------------- #
@st.cache_resource(show_spinner="Downloading & loading model…")
def load_model():
    # Downloads once, then served from local cache. For a PRIVATE repo, pass
    # token=st.secrets["HF_TOKEN"] here and add HF_TOKEN in Streamlit secrets.
...
def predict_los(X):
    # Predict via DMatrix rather than model.predict(). The sklearn wrapper's
    # default inplace path (XGBoosterPredictFromColumnar) can raise an
    # XGBoostError on frames that mix int and categorical columns with some
    # xgboost builds. The DMatrix path handles categoricals reliably and is the
    # same path used by shap_contributions() below, so results stay consistent.
    dm = xgb.DMatrix(X, enable_categorical=True)
    pred_log = float(booster.predict(dm, iteration_range=(0, BEST_IT + 1))[0])
